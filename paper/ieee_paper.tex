...
\begin{table}[t]
  ...
  \begin{tabular}{ll}
    ...
    $\phi_{k},p_{k}$         & Period-$k$ session share / price \\
...
\begin{equation}
\label{eq:lambda_k}
\lambda_{j,k} = \frac{\bar\mu_{j}\,\phi_{k}}{T_{k}},
\end{equation}
where $\bar\mu_{j}$ is the daily mean session count at station $j$,
$T_{k}$ is the period length, and $\phi_{k}$ is the period's share
of daily sessions derived from the ToU multipliers. Service is
exponential with rate $\mu{=}60/\tau$ per hour
($\tau{=}30$\,min mean session duration, consistent with a
...
$\rho_{k}\!=\!a_{k}/c_{j}$ the per-server utilisation. The
...
utilisation $\rho_{k}$ are also retained: a layout in which any
period satisfies $\rho_{k}\!\ge\!1$ is unstable (queue diverges in
...
effective per-period session shares $(\phi_{\text{pk}},\phi_{\text{nm}},
\phi_{\text{id}})$ derived from the ToU weights, the daily revenue of
station $j$ is
$R_{j}=\bar\mu_{j}\!\sum_{k}\!\phi_{k}p_{k}$.
...
f_{2}(\mathbf{x})&= -\frac{\sum_{w\in\mathcal{W}}D_{w}\,\mathbb{1}_{\left\{\exists j:\,x_{j}=1,\|s_{j}-w\|\le r_{z(w)}\right\}}}{\sum_{w\in\mathcal{W}}D_{w}}, \label{eq:f2}\\
...
\item \textbf{Optimisation layer.} Instantiates \texttt{EVStationProblem}
      ...
      $\tau_{\text{HV}}{=}5\!\times\!10^{-4}$ over $W{=}40$ consecutive
      windows.
...
\begin{algorithmic}[1]
  ...
  \FOR{$g\!=\!1,\dots,G$}
    ...
    \IF{$g\!\ge\!\max(W\!+\!1,100)$ and
        $(\max_{W}\!-\!\min_{W})/|\max_{W}|\!<\!\tau_{\text{HV}}$}
...
\begin{equation}
  \mathbf{v}_{i}^{(t+1)} = w\,\mathbf{v}_{i}^{(t)}
    + c_{1}\mathbf{r}_{1}\!\odot\!(\mathbf{p}_{i}\!-\!\mathbf{x}_{i})
    + c_{2}\mathbf{r}_{2}\!\odot\!(\mathbf{l}\!-\!\mathbf{x}_{i})
\end{equation}
with $\mathbf{r}_{1},\mathbf{r}_{2}\!\sim\!U[0,1]^{n}$, inertia
...
$\rho\!\ge\!1$ stability bound or the $P_{K}\!>\!10\%$ blocking
